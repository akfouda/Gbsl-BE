...
exports.getAll = (Model, modelName = "", populateOptions = []) =>
  asyncHandler(async (req, res) => {
    ...
    // Apply filter for approvedBy from query parameters if present
    if (req.query.approvedBy) {
      filter.approvedBy = req.query.approvedBy;
    }
    ...
    const documents = await mongooseQuery.populate(populateOptions);
